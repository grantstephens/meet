...
export default function Room({ name, participantName, admin }: RoomProps) {
  const roomName = name || '';
  ...
  useEffect(() => {
    // Check if participant name is provided
    if (!participantName && roomName) {
      console.log('No participant name provided, redirecting to home with room:', roomName);
      route(`/?room=${encodeURIComponent(roomName)}`, true);
      return;
    }
